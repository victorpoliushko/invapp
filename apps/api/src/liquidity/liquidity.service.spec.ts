...
import {
  computeLiquidity,
  mergeLiquiditySummaries,
  LiquidityPortfolioInput,
...
describe('computeLiquidity', () => {
  it('values a stock position at currentPrice * quantity with the US range by default', () => {
    ...
    portfolio.stockPositions = [
      { asset: { currentPrice: 100, exchange: null }, price: 50, quantity: 10 },
    ];

    const result = computeLiquidity(portfolio);

    expect(result.byType.stocks).toEqual({
      amount: 1000,
      ...STOCK_LIQUIDITY.US,
    });
  });
  ...
  it('classifies a recognized EU exchange into the EU range', () => {
    ...
    portfolio.stockPositions = [
      {
        asset: { currentPrice: 100, exchange: 'XETRA' },
        price: 50,
        quantity: 1,
      },
    ];
  ...
  it('falls back to the position price when the asset has no live currentPrice', () => {
    const portfolio = emptyPortfolio();
    portfolio.stockPositions = [
      { asset: { currentPrice: null, exchange: null }, price: 20, quantity: 5 },
    ];
  ...
  it('classifies a major-ticker crypto position into the MAJOR range', () => {
    const portfolio = emptyPortfolio();
    portfolio.cryptoPositions = [
      {
        asset: { currentPrice: 50000, ticker: 'btc' },
        price: 40000,
        quantity: 2,
      },
    ];
    ...
    expect(result.byType.crypto).toEqual({
      amount: 100000,
      ...CRYPTO_LIQUIDITY.MAJOR,
    });
  });
  ...
  it('classifies an unrecognized ticker as LONG_TAIL', () => {
    ...
    portfolio.cryptoPositions = [
      {
        asset: { currentPrice: 1, ticker: 'SOMENICHECOIN' },
        price: 1,
        quantity: 1000,
      },
    ];
  ...
  it('values a bond at currentValue * quantity, defaulting to investment-grade timing', () => {
    ...
    expect(result.byType.bonds).toEqual({
      amount: 1050,
      ...BOND_LIQUIDITY.CORPORATE_INVESTMENT_GRADE,
    });
  });
  ...
  it('gives real estate both a fast-sale headline range and a full-value alternative', () => {
    const portfolio = emptyPortfolio();
    portfolio.realEstateAssets = [
      { currentValue: 100000, purchasePrice: 90000, type: 'APARTMENT' },
    ];
    const expected = realEstateLiquidity('APARTMENT', 100000);

    const result = computeLiquidity(portfolio);

    expect(result.byType.realEstate).toEqual({
      amount: 100000,
      ...expected.fast,
      full: { amount: 100000, ...expected.full },
    });
  });
  ...
  it('sums the liquid bucket across stocks, bonds, and crypto with a widened min/max range', () => {
    const portfolio = emptyPortfolio();
    portfolio.stockPositions = [
      { asset: { currentPrice: 100, exchange: null }, price: 100, quantity: 1 },
    ]; // US: 1-2
    portfolio.bonds = [{ currentValue: 100, purchasePrice: 100, quantity: 1 }]; // investment grade: 2-4
    portfolio.cryptoPositions = [
      { asset: { currentPrice: 100, ticker: 'BTC' }, price: 100, quantity: 1 },
    ]; // major: 0-1
...
describe('mergeLiquiditySummaries', () => {
  it('sums amounts and widens ranges across multiple portfolios', () => {
    const a = computeLiquidity({
      ...
      stockPositions: [
        {
          asset: { currentPrice: 100, exchange: null },
          price: 100,
          quantity: 1,
        },
      ], // US: 1-2
    ...
    expect(merged.byType.stocks).toEqual({
      amount: 200,
      minDays: 1,
      maxDays: 3,
    });
